                    ...
                    <form method="POST" action="{{ route("admin.services.update", [$service->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="status" id="status" value="1" {{ $service->status || old('status', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('orderby') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" min="1" oninput="this.value = Math.abs(this.value) "name="orderby" id="orderby" value="{{ old('orderby', $service->orderby) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('special_offer') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="special_offer" id="special_offer" value="1" {{ $service->special_offer || old('special_offer', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('timetable') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="timetable" id="timetable" value="1" {{ $service->timetable || old('timetable', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('branch') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="branch_id" id="branch_id" required>
                                @foreach($branches as $id => $branch)
                                    <option value="{{ $id }}" {{ ($service->branch ? $service->branch->id : old('branch_id')) == $id ? 'selected' : '' }}>{{ $branch }}</option>
                                @endforeach
                            </select>
