        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.trainingSchedule.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.training-schedules.update", [$trainingSchedule->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('service') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="service_id" id="service_id" required>
                                @foreach($services as $id => $service)
                                    <option value="{{ $id }}" {{ ($trainingSchedule->service ? $trainingSchedule->service->id : old('service_id')) == $id ? 'selected' : '' }}>{{ $service }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('day_of_week') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="day_of_week" id="day_of_week" required>
                                ...
                                @foreach(App\TrainingSchedule::DAY_WEEK_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('day_of_week', $trainingSchedule->day_of_week) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('time') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="time" name="time" id="time" value="{{ old('time', $trainingSchedule->time) }}" required>
                            @if($errors->has('time'))
                        ...
                        <div class="form-group {{ $errors->has('treainer') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="treainer_id" id="treainer_id" required>
                                @foreach($treainers as $id => $treainer)
                                    <option value="{{ $id }}" {{ ($trainingSchedule->treainer ? $trainingSchedule->treainer->id : old('treainer_id')) == $id ? 'selected' : '' }}>{{ $treainer }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('training_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="training_name" id="training_name" value="{{ old('training_name', $trainingSchedule->training_name) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('pay_type_training') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="pay_type_training" id="pay_type_training" required>
                                ...
                                @foreach(App\TrainingSchedule::PAY_TYPE_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('pay_type_training', $trainingSchedule->pay_type_training) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('stock') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="stock_id" id="stock_id">
                                @foreach($stocks as $id => $stock)
                                    <option value="{{ $id }}" {{ ($trainingSchedule->stock ? $trainingSchedule->stock->id : old('stock_id')) == $id ? 'selected' : '' }}>{{ $stock }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="status" id="status">
                                ...
                                @foreach(App\TrainingSchedule::STATUS_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('status', $trainingSchedule->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('branch') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="branch_id" id="branch_id" required>
                                @foreach($branches as $id => $branch)
                                    <option value="{{ $id }}" {{ ($trainingSchedule->branch ? $trainingSchedule->branch->id : old('branch_id')) == $id ? 'selected' : '' }}>{{ $branch }}</option>
                                @endforeach
                            </select>
